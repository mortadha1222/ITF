...
{
    QSqlQuery query;
    QString res0 =QString::number(id_ouvrier);
    QString res =QString::number(salaire);
    QString res1 = QString::number(cin);
    QString res2 = QString::number(num_tel);
    query.prepare(" UPDATE ouvriers SET nom=:nom, prenom=:prenom, ROLE=:role, date_debut=:date_debut, date_fin=:date_fin, salaire=:salaire, cin=:cin ,NUM_TEL=:num_tel WHERE id_ouvrier=:id_ouvrier ");
    query.bindValue(":id_ouvrier",res0);
    query.bindValue(":nom",nom);
    query.bindValue(":prenom",prenom);
    query.bindValue(":role",role);
    query.bindValue(":salaire",res);
